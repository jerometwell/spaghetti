...
class Container {
  ...
  getMemoizedOrCreate(reg) {
    const existing = reg.instance;
    if (existing) {
      return existing;
    }
    const created = this.create(reg);
  ...
  _resolveSingle(reg) {
    if(this._scope) {
      if(reg.lifecycle === Registration.SCOPED) {
        return this.getMemoizedOrCreate(reg);
      } else if( reg.lifecycle === Registration.SINGLETON) {
        return this._parent.getMemoizedOrCreate(reg);
      } else if (reg.lifecycle === Registration.TRANSIENT) {
        return this._parent.create(reg);
      } else {
        throw new Error("scope, unknown lifecycle " + reg.lifecycle)
      }
    } else {
      if (reg.lifecycle === Registration.SINGLETON || reg.lifecycle === Registration.SCOPED) {
        return this.getMemoizedOrCreate(reg);
      } else if (reg.lifecycle === Registration.TRANSIENT) {
  ...
  _register(lifecycle, labels, ctor, options = {}) {
    const {isCtor, wiring} = options;
    // normalize
    if (!Array.isArray(labels)) {
      labels = [labels];
    }
    if(wiring) {
      this.wire(ctor, wiring);
    }

    this._registrations.push( new Registration(ctor, lifecycle, labels, {isCtor} ));
  }

  singleton = (labels, ctor, options) => this._register(Registration.SINGLETON, labels, ctor, options);
  transient = (labels, ctor, options) => this._register(Registration.TRANSIENT, labels, ctor, options);
  scoped    = (labels, ctor, options) => this._register(Registration.SCOPED,    labels, ctor, options);

  singletonFn = (labels, ctor, options) => this._register(Registration.SINGLETON, labels, ctor, {...options, isCtor: false});
  transientFn = (labels, ctor, options) => this._register(Registration.TRANSIENT, labels, ctor, {...options, isCtor: false});
  scopedFn    = (labels, ctor, options) => this._register(Registration.SCOPED,    labels, ctor, {...options, isCtor: false});
